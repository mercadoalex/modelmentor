...
import { FeatureImportanceRanking } from '@/components/model/FeatureImportanceRanking';
...
export default function TrainingPage() {
  ...
  // Enhanced training pipeline state
  ...
  const startTraining = async () => {
    ...
    try {
      ...
      const pipeline = new EnhancedTrainingPipeline({
        ...
        onProgress: (progress) => {
          // Progress is handled by epoch updates
        },
        onEpochEnd: async (epoch, epochLogs) => {
          ...
          const remaining = avgTimePerEpoch * (trainingConfig.epochs - epoch);
        ...
        onMetricsUpdate: (metricsUpdate) => {
          setCurrentMetrics(prev => ({
            ...
            bestLoss: prev.bestLoss === undefined || metricsUpdate.loss < prev.bestLoss 
              ? metricsUpdate.loss 
              : prev.bestLoss,
            bestAccuracy: prev.bestAccuracy === undefined || (metricsUpdate.accuracy && metricsUpdate.accuracy > prev.bestAccuracy)
      ...
      // Parse dataset data - for demo purposes, create sample data based on model type
      ...
      // For numeric data, use enhanced pipeline
      if (project.model_type === 'regression' || (Array.isArray(trainingData[0].input) && typeof trainingData[0].output === 'number')) {
        ...
        const dataForPipeline = trainingData.map((d, i) => {
          ...
          if (Array.isArray(d.input)) {
            d.input.forEach((val, idx) => {
              row[`feature_${idx}`] = val as number;
            });
          } else {
        ...
        // For text classification, use existing training functions
        const config = {
          epochs: trainingConfig.epochs,
        ...
        const callbacks = {
          onEpochEnd: async (epoch: number, epochLogs: { loss: number; accuracy: number; val_loss?: number; val_accuracy?: number }) => {
            if (trainingCancelledRef.current) return;
            ...
            const remaining = avgTimePerEpoch * (trainingConfig.epochs - epoch - 1);
            setEstimatedTimeRemaining(Math.max(0, remaining));
            ...
            setCurrentMetrics(prev => ({
              ...
              bestLoss: prev.bestLoss === undefined || epochLogs.loss < prev.bestLoss 
                ? epochLogs.loss 
                : prev.bestLoss,
              bestAccuracy: prev.bestAccuracy === undefined || epochLogs.accuracy > prev.bestAccuracy
                ? epochLogs.accuracy
                : prev.bestAccuracy,
            }));
  ...
  const stopTraining = async (completed = false) => {
    ...
    if (trainingSession && completed) {
      ...
      if (projectId) {
        await projectService.update(projectId, { status: 'testing' });
      }
      
      // Track training completion for registered users
      if (user && project && projectId && trainingStartTime) {
        ...
        // Create in-app notification for training completion
        await supabase
          .from('notifications')
          .insert({
            user_id: user.id,
            ...
            message: `Your model "${project.title}" has finished training with ${finalAccuracy ? `${(finalAccuracy * 100).toFixed(1)}% accuracy` : 'completion'}!`,
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      const trainingHistory = metrics.map(m => ({
        epoch: m.epoch,
        loss: m.loss,
        accuracy: m.accuracy,
      }));
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {/* Left Column - Configuration and Stage Indicator */}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {!isTraining && !isCompleted && dataset && (
              <CrossValidationPreview
                datasetSize={dataset.sample_count || 100}
                numClasses={dataset.labels?.length || 2}
                onRecommendationApply={(ratio) => {
                  setTrainingConfig({
                    ...trainingConfig,
                    validationSplit: 1 - ratio,
                  });
            ...
            {/* Transfer Learning */}
            {!isTraining && !isCompleted && dataset && project?.model_type === 'image_classification' && (
              <TransferLearningPanel
                datasetSize={dataset.sample_count || 100}
                targetClasses={dataset.labels?.length || 2}
                onEnableTransferLearning={(enabled, model) => {
                  if (enabled && model) {
                    toast.success(`Transfer learning enabled with ${model.name}`);
                  }
                }}
              />
            )}
            ...
            {!isTraining && !isCompleted && dataset && (
              <ModelComparisonPanel
                featureCount={dataset.labels?.length || 5}
                sampleCount={dataset.sample_count || 100}
                problemType={project?.model_type === 'regression' ? 'regression' : 'classification'}
              />
            )}

            {/* Multi-Model Comparison Dashboard */}
            {!isTraining && !isCompleted && dataset?.labels && (
            ...
            {!isTraining && !isCompleted && dataset && (
              <LearningCurveAnalysis
                totalSamples={dataset.sample_count || 100}
                features={dataset.labels?.length || 5}
              />
            )}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {isCompleted ? 'Training completed' : isTraining ? 'Training in progress' : 'Ready to start training'}
                </CardDescription>
              </CardHeader>
            ...
            {metrics.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Real-time accuracy and loss visualization</CardDescription>
                </CardHeader>
                <CardContent>
                  <LineChart
                    data={{
            ...
            {/* Training Completion Summary - Educational Overview */}
            ...
            {/* Confusion Matrix */}
            {isCompleted && project?.model_type !== 'regression' && dataset?.labels && (
              <ConfusionMatrixVisualization
                matrixData={confusionMatrixService.generateConfusionMatrix(
                  currentMetrics.bestAccuracy || 0.85,
                  dataset.labels,
            ...
            {isCompleted && project?.model_type !== 'regression' && dataset?.labels && (
              <ErrorAnalysisDashboard
                errorAnalysis={errorAnalysisService.generateErrorAnalysis(
                  currentMetrics.bestAccuracy || 0.85,
                  dataset.labels,
                  dataset.labels.map((_, i) => `Feature_${i + 1}`),
                  dataset.sample_count || 100
                )}
              />
            )}
            ...
            {isCompleted && (
              <ModelDeploymentGuide
                modelName={project?.title || 'my_model'}
              />
            )}
            ...
            {isCompleted && dataset?.labels && (
              <ModelPlayground
                features={dataset.labels}
                featureImportance={dataset.labels.map((_, index) => 
